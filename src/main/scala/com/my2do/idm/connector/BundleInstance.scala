...
class BundleInstance(@BeanProperty var bundle: Bundle)  {
	val log = Logger("vidm")
  ...
  // The API config - used to get a facade connection
  lazy val connectorAPIConfiguration: APIConfiguration = ConnectorManager.instance.apiConfigForKey(bundle.connectorKey)
